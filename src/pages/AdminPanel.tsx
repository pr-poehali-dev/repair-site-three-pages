...
export default function AdminPanel() {
  ...
  const handleTabChange = (t: Tab) => {
    setTab(t);
    if (t === "foremen" && !foremensLoaded) loadForemen(pwd);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: "var(--brand-cream)" }}>
        <div className="w-full max-w-sm p-8 rounded-lg space-y-3" style={{ background: "var(--brand-warm-white)", border: "1px solid rgba(74,70,64,0.12)" }}>
          <h1 className="font-display font-bold text-xl" style={{ color: "var(--brand-dark)" }}>Кабинет управленца</h1>
          <input type="password" className={inputCls} style={inputStyle} placeholder="Пароль управленца" value={loginPwd} onChange={(e) => setLoginPwd(e.target.value)} onKeyDown={(e) => e.key === "Enter" && login()} />
